...
for(i in 1:length(occDataList)){
  ...
  myRespName = paste('Eremotherium_laurillardi_instancia', i, sep='')
  ...
  save(myBiomodData, file=paste(projectFolder, '/', gsub('_', '.', myRespName), '/', gsub('_', '.', myRespName), '.myBiomodData.R', sep=''))
  save(myBiomodOption, file=paste(projectFolder,'/', gsub('_', '.', myRespName), '/', gsub('_', '.', myRespName),'.myBiomodOption.R',sep=''))
  save(myBiomodModelOut, file=paste(projectFolder,'/', gsub('_', '.', myRespName), '/', gsub('_', '.', myRespName),'.myBiomodModelOut.R',sep=''))
...
for(i in seq(length(occDataList))){
  for(year in 1:5){ #trabalhando com intervalo entre 1 e 5 mil anos, como exemplo
    ...
    envVarPaths = list.files( file.path(envFolder, year), full.names=TRUE) # vamos projetar para 5  mil anos atras (ultimo maximo glacial)
    predictors = stack(envVarPaths) #abrindo e armazenando as variaveis para dentro do R
    myRespName = paste('Eremotherium_laurillardi_instancia', i, sep='')
    
    # carregando dados
    load(paste(projectFolder, '/', gsub('_', '.', myRespName), '/', gsub('_', '.', myRespName), '.myBiomodData.R', sep=''))
    load(paste(projectFolder,'/', gsub('_', '.', myRespName), '/', gsub('_', '.', myRespName),'.myBiomodOption.R',sep=''))
    ...
    writeRaster(x = rasterData, filename = paste(projectFolder, '/', gsub('_', '.', myRespName), '/proj', year, '.asc', sep=''), overwrite=TRUE)
  }
}


### analisando as projecoes
number_of_instances = 3  # ajustar aqui, de acordo com o que tiver sido usado nas etapas anteriores 
number_of_projections = 5  # ajustar aqui, de acordo com o que tiver sido usado nas etapas anteriores 
raster_list = list()
...
for(year in seq(number_of_projections)){
  ...
  for(i in seq(number_of_instances)){
    myRespName = paste('Eremotherium_laurillardi_instancia', i, sep='')
    filepaths = append(filepaths, paste(projectFolder, '/', gsub('_', '.', myRespName), '/proj', year, '.asc', sep=''))
    filenames = append(filenames, paste('instance', i, '_proj', year, sep=''))
  }
