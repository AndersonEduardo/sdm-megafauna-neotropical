...
paleobg = function(x, colNames=names(x), envFolder, n=10000){
  ...
  for ( age_i in unique(ages) ){
    ...
    if(age_i %in% as.numeric(list.files(envFolder))){
      envData = list.files(file.path(envFolder,age_i), full.names=TRUE, pattern='.asc')
      varNames = basename(list.files(file.path(envFolder,age_i), full.names=TRUE, pattern='.asc'))
    ...
    bgData_i_vars = raster::extract(envData, bgData_i[,c('lon','lat')])
