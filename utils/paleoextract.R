...
paleoextract = function(x, cols=names(x), path) {
  ...
  idNA = sapply( 
    seq(nrow(currentDataSet)),
    function(x) currentDataSet[x,]$age  %in% as.numeric(list.files(path))
  )
  ageNA = sapply(
    seq(length(age)), 
    function(x) age[x]  %in% as.numeric(list.files(path))
  )
  ...
  for (age_i in age[ageNA]){
    ...
    varValues = raster::extract(x = currentPredictors, y = currentCoords)
  ...
  if ( sum(idNA==FALSE) > 0 ){ 
    ...
    # colunas com variaveis ambientais
    envirDataCols = grep(paste(c(cols,"ID"), collapse="|"), names(predictorsData), value=TRUE, invert=TRUE)
    # matriz deNAs
    matrixNA = matrix(NA, nrow=nrow(currentDataSet[!idNA,]), ncol=length(envirDataCols))
    # dados associados a matriz de NAs
    naData = cbind(currentDataSet[!idNA,], as.data.frame(matrixNA))
    # ajuste de nomes das colunas
    names(naData) = names(predictorsData)
    # juntando NAs e dados obtidos ("nao-NAs")
    predictorsData = rbind(predictorsData, naData)
